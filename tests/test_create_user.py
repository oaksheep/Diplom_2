...
class TestCreateUser:
    ...
    def test_create_new_user(self):
        response = requests.post(f'{Urls.MAIN_URL}{Endpoints.CREATE_USER}', data=register_new_user())
        assert response.status_code == 200 and response.json()["success"] is True

    @allure.title('Создание уже существующего пользователя')
    def test_create_user_double(self):
        response = requests.post(f'{Urls.MAIN_URL}{Endpoints.CREATE_USER}', data=data_double)
    ...
    def test_create_user_fail(self, data):
        response = requests.post(f'{Urls.MAIN_URL}{Endpoints.CREATE_USER}', data=data)
