...
class TestCreateOrder:
    ...
    def test_create_order_true(self):
        response = requests.post(Urls.MAIN_URL + Endpoints.CREATE_ORDER, headers=Endpoints.headers, json=data_ingredients)
        assert response.status_code == 200 and response.json()["success"] is True

    @allure.title("Создания заказа без авторизации без ингредиентов")
    def test_create_order_not_ingredients(self):
        response = requests.post(Urls.MAIN_URL + Endpoints.CREATE_ORDER, headers=Endpoints.headers,
